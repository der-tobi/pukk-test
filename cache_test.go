...
func TestBookingCacheSnapshotInfersActualReturnedFreeBusyInterval(t *testing.T) {
	now := time.Date(2026, 8, 14, 0, 40, 0, 0, time.UTC)
	rooms := &recordingRooms{
		freeBusy:      "01110",
		freeBusyExact: true,
	}
...
func TestBookingCacheSnapshotTreatsRoomsStatusFourAsBusy(t *testing.T) {
	now := time.Date(2026, 8, 13, 10, 0, 0, 0, time.UTC)
	rooms := &recordingRooms{
		freeBusy:      "000444000000000",
		freeBusyExact: true,
	}
	cache := NewBookingCache(rooms, CacheConfig{
		ResourceID:      44,
	...
	assertSnapshotPattern(t, snapshot, "GGGRRRGGGGGG")
}
