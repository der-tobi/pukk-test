...
func TestPollRendersFutureHalfHourBookingAfterTwentyFreeMinutes(t *testing.T) {
	now := time.Date(2026, 8, 14, 0, 40, 0, 0, time.UTC)
	rooms := &recordingRooms{
		freeBusy: minuteFreeBusy(75, rangeInts(20, 49)...),
		bookings: []Booking{{
			ID:               7,
			ResourceID:       44,
			Start:            time.Date(2026, 8, 14, 1, 0, 0, 0, time.UTC),
			End:              time.Date(2026, 8, 14, 1, 30, 0, 0, time.UTC),
			CheckinConfirmed: true,
		}},
	}
	app := NewApp(rooms, AppConfig{
