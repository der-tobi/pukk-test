...
- Per live-device simplification, the normal ring uses red `#FF0000` for the first visible busy/booked range, violet `#7D00B3` for later busy/booked ranges, and bright green `#00FF00` for free. Pending button selections render blue `#006DFF` during the commit window, then turn red or violet by their final busy-range position after commit. The earlier orange check-in, gradient, and pulsing colors are disabled for the PoC.
...
- `app.go` implements short-press extend/ad-hoc selection with a 5s commit window, NFC checkin, and 3s longpress release/checkout. Longpress checkout pushes a blue sweep over the current booking LEDs from the meeting end back toward now, releases the booking in Rooms, then sweeps those LEDs green.
...
- PuKK device-local REST push is implemented for the ambient ring on every poll and for the longpress checkout blue-to-green sweep. Dedicated faster NFC/checkin animations are still not implemented.
