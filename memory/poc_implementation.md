...
- `cache.go` over-fetches `[now bucket, now bucket + 75min]` by default, requests Rooms `freebusy` at 1-minute resolution, infers the actual returned source interval from bitstring length when Rooms returns 1/5/15-minute buckets, aggregates that into 12 rolling five-minute LEDs from the current poll time, and treats unknown slices as busy. In normal successful refreshes, exact booking ranges from `bookings/find` decide red booked slots; freebusy remains fallback/diagnostic data.
...
- `/debug/status` includes `availabilityBits`, `availabilityStart`, `availabilityEnd`, and `lastAvailabilityRefreshOk` after the FreeBusy cache refresh succeeds.
