...
- Per live-device simplification, the normal ring uses orange `#FF9D09` for the current booking, red `#FF0000` for the first visible busy/booked range when there is no current booking, violet `#7D00B3` for later/future busy ranges, and bright green `#00FF00` for free. Pending button selections render blue `#006DFF` during the commit window, then orange/red/violet by their final busy-range position after commit. The earlier gradient and pulsing colors are disabled for the PoC.
