...
- `app.go` implements short-press extend/ad-hoc selection with a 5s commit window, NFC checkin, and 3s longpress release/checkout. NFC check-in pushes an orange-to-red sweep over the current booking LEDs after Rooms confirms check-in. Longpress checkout pushes a blue sweep over the current booking LEDs from the meeting end back toward now, releases the booking in Rooms, then sweeps those LEDs green.
...
- PuKK device-local REST push is implemented for the ambient ring on every poll, NFC check-in orange-to-red sweep, and longpress checkout blue-to-green sweep.
