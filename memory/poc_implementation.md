...
- Per live-device simplification, the normal ring uses vivid orange `#FF6A00` for current-booking LEDs only while `checkinConfirmed=false`, red `#FF0000` for checked-in/current busy LEDs, violet `#7D00B3` for later/future busy ranges, and bright green `#00FF00` for free. Pending button selections render blue `#006DFF` during the commit window, then red/violet by their final busy-range position after commit. The earlier gradient and pulsing colors are disabled for the PoC.
- Active/current slots are tracked separately from generic busy slots, so a follow-up booking stays violet even when the current booking has been extended to end exactly at the follow-up start.
- Checked-in current bookings with five minutes or less remaining render only the current LED in disco mode, cycling bright pink/cyan/white/purple at the normal poll cadence. Unchecked bookings remain orange, and provisional blue still overrides disco during button feedback.
