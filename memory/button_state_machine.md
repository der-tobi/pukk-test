...
**NFC:** accepts any tag, no UID/card validation (matches the PoC's "hardcode everything" framing). Current-booking LEDs render orange only while `checkinConfirmed=false`; checked-in current bookings render red. An NFC tap checks in the current booking through legacy v1 `PUT bookings/{id}/checkin` as the authenticated API/service user, deliberately without a `pin` query parameter. On success, the visible current-booking LEDs sweep orange to red through the PuKK local REST API. The failed `bookings/{id}/checkin?pin=...` test showed that the startup API password is not the right PIN: local Rooms source checks non-empty pins against reservation `DoormanList` PINs. The documented PuKK remote-server event model exposes a single `nfc` event with card data, not a tap duration or card-present/card-removed pair, so an NFC 3-second hold cannot be implemented unless live hardware provides repeated/undocumented signals.
