...
**NFC:** accepts any tag, no UID/card validation (matches the PoC's "hardcode everything" framing). Checks in the current booking using the same 3V Rooms password captured at startup, passed as the `pin` query param on the checkin call ([[rooms-api-integration]]) — relies on that account having checkin rights for resource 44. A current booking with `checkinConfirmed=false` renders orange; after a successful NFC check-in, the current-booking LEDs sweep orange to red via the device-local REST API. No-op if there's no current booking or it's already checked in. The documented PuKK remote-server event model exposes a single `nfc` event with card data, not a tap duration or card-present/card-removed pair, so an NFC 3-second hold cannot be implemented unless live hardware provides repeated/undocumented signals.
