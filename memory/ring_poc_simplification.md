...
- Busy/booked/provisional = red `#FF0000`
- Free = bright green `#00FF00`

Do not use the earlier blue selection color, orange check-in color, violet upcoming-booking color, red-to-green gradient, or brightness pulsing in the normal poll-rendered ring. The original `#00FF7F` free color looked blue/turquoise on the physical device and was replaced with pure green `#00FF00`; the original red was replaced with pure red `#FF0000`.
...
- 1 press renders the next 15 minutes red.
- 2 presses render the next 30 minutes red.
- 3 presses render the next 45 minutes red.
- Remaining visible time stays green.

On button presses during an active booking, the already-booked remaining time and the provisional extension both render red. Extension is capped by `now + 60min`, not by a wall-clock block from the booking's original start. Example: a 17:00-18:00 booking pressed at 17:10 extends to 18:10 when no later booking blocks the hour.
