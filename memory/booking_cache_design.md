...
**Exact interaction cap (updated 2026-08-17):** exact `bookings/find` ranges are now used not only as a render overlay but also as the authoritative headroom cap for button extension/ad-hoc selection. This lets interactions fill a free gap up to the exact next booking start, even when the gap is not a multiple of 15 minutes. Example: if there are 23 minutes until the next booking, two button presses commit an ad-hoc booking or extension ending at that exact 23-minute boundary. Button handling now refreshes exact bookings on demand when the exact cache is not known yet. If exact booking lookup still fails, interactions no longer fall back to `freebusy` as a veto; the Rooms create/extend API is allowed to accept or reject the mutation.

**Freebusy is not user-specific bookability (2026-08-17):** local 3V Rooms source tests show `freebusy` can encode closed opening hours and blocking times as `1`, while definitive bookings can appear as status `4`, despite the public Swagger describing a binary string. The current API user can book outside opening hours, so `freebusy` must not be treated as the local authority for whether button-created bookings are allowed. The app still parses any non-zero `freebusy` status as busy for fallback/diagnostic rendering, but interactions rely on exact bookings plus the Rooms mutation endpoint.
