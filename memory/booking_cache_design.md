...
**Exact interaction cap (implemented 2026-08-17):** exact `bookings/find` ranges are now used not only as a render overlay but also as the authoritative headroom cap for button extension/ad-hoc selection when the exact lookup succeeds. This lets interactions fill a free gap up to the exact next booking start, even when the gap is not a multiple of 15 minutes. Example: if there are 23 minutes until the next booking, two button presses commit an ad-hoc booking or extension ending at that exact 23-minute boundary. If exact ranges are unknown because the lookup failed, the app falls back to the coarser cached `freebusy` slot logic.
